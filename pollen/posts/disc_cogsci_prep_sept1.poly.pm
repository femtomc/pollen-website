...
I will not argue about the content of representations in physics, suffice it to say that different base representations can lead to drastically different theories (think: N-dimensional Euclidean space, Hilbert space, even differentiable manifolds which are locally Minkowski). Instead, I'd like to focus in on the usage of ◊em{computation} in ◊em{mental computation} - which invokes a stronger relationship with the dynamics described by information theory, and (in the context of physics), something like the study of natural computation or quantum computation.
